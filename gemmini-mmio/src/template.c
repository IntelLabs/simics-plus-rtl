...
int main() {
  ...
  size_t in_pcie = DIM * DIM * sizeof(elem_t);
  size_t identity_pcie = 2 * DIM * DIM * sizeof(elem_t);
  size_t out_pcie = 3 * DIM * DIM * sizeof(elem_t);

  printf("Move \"In\" matrix from main memory into Gemmini's scratchpad\n");
  gemmini_config_ld(DIM * sizeof(elem_t));
  gemmini_config_st(DIM * sizeof(elem_t));

  // memcpy_to_pcie(in_pcie, (uint8_t *) In, DIM * DIM * sizeof(elem_t));
  gemmini_mvin(In, In_sp_addr);

  printf("Move \"Identity\" matrix from main memory into Gemmini's scratchpad\n");
  // memcpy_to_pcie(identity_pcie, (uint8_t *) Identity, DIM * DIM * sizeof(elem_t));
  ...
  gemmini_fence();
  // memcpy_from_pcie((uint8_t *) Out, out_pcie, DIM * DIM * sizeof(elem_t));
