...
#define RAND rand()
    ...
    size_t a_pcie = 64;
    size_t b_pcie = a_pcie + MAT_DIM_I * MAT_DIM_K * sizeof(elem_t);
    size_t c_pcie = b_pcie + MAT_DIM_K * MAT_DIM_J * sizeof(elem_t);
    size_t d_pcie = c_pcie + MAT_DIM_I * MAT_DIM_J * sizeof(elem_t);

    // memcpy_to_pcie(a_pcie, (uint8_t *) full_A, MAT_DIM_I * MAT_DIM_K * sizeof(elem_t));
    // memcpy_to_pcie(b_pcie, (uint8_t *) full_B, MAT_DIM_K * MAT_DIM_J * sizeof(elem_t));
    // memcpy_to_pcie(d_pcie, (uint8_t *) full_D, MAT_DIM_I * MAT_DIM_J * sizeof(ACC_T));

    printf("Starting gemmini matmul\n");
    unsigned long start = read_cycles();

    tiled_matmul_auto(MAT_DIM_I, MAT_DIM_J, MAT_DIM_K,
            (elem_t*)full_A, (elem_t*)full_B, NO_BIAS ? NULL : full_D, (elem_t*)full_C,
    ...
    // memcpy_from_pcie((uint8_t *) full_C, c_pcie, MAT_DIM_I * MAT_DIM_J * sizeof(elem_t));
